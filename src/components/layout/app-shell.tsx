...
import { AgentTriggerButton } from '@/components/agent-workspace/agent-trigger-button'
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // When the split-screen agent workspace is open, suppress the mobile bottom
  // nav and the command-palette FAB — they would otherwise overlap the agent
  // panel's right-hand chrome. Desktop sidebar + header remain available
  // inside the dashboard panel on the left.
  const suppressMobileChrome = focusMode || agentWorkspaceOpen
    ...
    <>
      <AgentWorkspace>
        <div className="flex min-h-screen bg-background">
          ...
          {/* Mobile bottom nav */}
          {!suppressMobileChrome && <MobileNav />}
      ...
      {/* Floating "Run Hermes Agent Automation" trigger — sits above both
          panels via fixed positioning so it doubles as the "close" button
